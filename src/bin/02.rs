...
pub fn part_two(input: &str) -> Option<u32> {
    let mut safe = 0;
    for line in input.lines() {
        let sequence: Vec<i32> = line.split_whitespace().map(|s| s.parse::<i32>().unwrap()).collect();
        ...
        for i in 0..sequence.len() {
            ...
            if is_safely_increasing_sequence(skipper.clone()) || is_safely_decreasing_sequence(skipper.clone()) {
                safe += 1;
...
mod tests {
    ...
    fn test_is_safely_increasing() {
        ...
        let sequence: Vec<i32> = input.lines().last().unwrap().split_whitespace().map(|s| s.parse::<i32>().unwrap()).collect();
    ...
    fn test_is_safely_decreasing() {
        let input = advent_of_code::template::read_file("examples", DAY);
        let sequence: Vec<i32> = input.lines().next().unwrap().split_whitespace().map(|s| s.parse::<i32>().unwrap()).collect();
